...
export function BudgetProgressRing() {
  const { budget, expenses, formatMoney, formatMoneyCompact } = useAppData();
  const { t } = useTranslation();
  ...
  const pct = budget > 0 ? Math.min(expenses / budget, 1) : 0;
  const offset = circumference * (1 - pct);
  const remaining = Math.max(budget - expenses, 0);
  const overBudget = expenses > budget;

  return (
    <div className="leaf-fill dw-bpr-panel">
      <div className="dw-col-header">
        <h3 className="dw-heading">{t("dashboard.budget")}</h3>
        <button className="dw-add-btn dw-add-btn-neutral" onClick={() => setEditing(true)}>{t("common.edit")}</button>
      </div>

      <div className="dw-bpr-body">
        <div className="dw-bpr-ring-wrap">
          ...
          <div className="dw-bpr-center">
            <span className="dw-bpr-center-value" title={formatMoney(remaining)}>{formatMoneyCompact(remaining)}</span>
            <span className="dw-bpr-center-label">{overBudget ? t("dashboard.over") : t("dashboard.left")}</span>
          </div>
        </div>

        <div className="dw-bpr-stats">
          <div className="dw-bpr-stat">
            <span className="dw-bpr-stat-label">{t("common.spent")}</span>
            <span className="dw-bpr-stat-value">{formatMoney(expenses)}</span>
          </div>
          <div className="dw-bpr-stat">
            <span className="dw-bpr-stat-label">{t("budgetTab.budget")}</span>
            <span className="dw-bpr-stat-value">{formatMoney(budget)}</span>
          </div>
          <div className="dw-bpr-stat">
            <span className="dw-bpr-stat-label">{t("budgetTab.remaining")}</span>
            <span className={"dw-bpr-stat-value" + (overBudget ? " dw-bpr-stat-over" : "")}>
      ...
      {editing && (
        <ModalShell title={t("budgetTab.editMonthlyBudget")} onClose={() => setEditing(false)}>
          <BudgetForm onClose={() => setEditing(false)} initialLimit={budget} />
        </ModalShell>
      )}
    </div>
  );
